...
class Cx extends TagLib
{
    ...
    protected $tags = array(
        ...
        'range' => array(
            'attr' => 'name,value,type',
            'level' => 3,
            'alias' => 'in,notin,between,notbetween'
        ),
        'empty' => array(
            'attr' => 'name',
            'level' => 3
        ...
        'for' => array(
            'attr' => 'start,end,name,comparison,step',
            'level' => 3
        )
    );
    ...
    public function _import($tag, $content, $isFile = false, $type = '')
    {
        ...
        if ($isFile) {
            ...
            foreach ($array as $val) {
                ...
                switch ($type) {
                    case 'js':
                        $parseStr .= '<script type="text/javascript" src="' . $basepath . '/' . str_replace(array(
                            '.',
                            '#'
                        ), array(
                            '/',
                            '.'
                        ), $val) . '.js' . ($version ? '?' . $version : '') . '"></script>';
                        break;
                    case 'css':
                        $parseStr .= '<link rel="stylesheet" type="text/css" href="' . $basepath . '/' . str_replace(array(
                            '.',
                            '#'
                        ), array(
                            '/',
                            '.'
                        ), $val) . '.css' . ($version ? '?' . $version : '') . '" />';
                        break;
                    case 'php':
