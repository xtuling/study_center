...
class RightService extends AbstractService
{
     ...
     * @return array
     */
    public function getReadList($user)
    {
        $rights = $this->getUserRight($user);
        $data = $this->_d->listByRight($rights, Constant::RIGHT_TYPE_IS_READ);
     ...
     * @return array
     */
    public function getDownloadList($user)
    {
        $rights = $this->getUserRight($user);
        $data = $this->_d->listByRight($rights, Constant::RIGHT_TYPE_IS_DOWNLOAD);
     ...
     * @return bool
     */
    public function checkReadRight($user, $file_id)
    {
        $rights = $this->getUserRight($user);
        $count = $this->_d->countByRight($file_id, $rights, Constant::RIGHT_TYPE_IS_READ);
        return $count > 0;
    }
     ...
     * @return bool
     */
    public function checkDownloadRight($user, $file_id)
    {
        if ($file_id == 0) {
            return true;
        }
        $rights = $this->getUserRight($user);
        $count = $this->_d->countByRight($file_id, $rights, Constant::RIGHT_TYPE_IS_DOWNLOAD);
        return $count > 0;
    }
}
