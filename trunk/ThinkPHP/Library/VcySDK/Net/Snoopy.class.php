...
class Snoopy {
					...
					if ($this->_redirectaddr) {
						...
						if ($this->maxredirs > $this->_redirectdepth) {
							...
							if (preg_match("|^https?://" . preg_quote($this->host) . "|i", $this->_redirectaddr) || $this->offsiteok) {
								...
								$this->_redirectdepth ++;
								$this->lastredirectaddr = $this->_redirectaddr;
								if (strpos($this->_redirectaddr, "?") > 0) $this->fetch($this->_redirectaddr); // the redirect has changed the request method from post to get
	 ...
	 */
	function set_submit_normal($type = '') {

		$type = empty($type) ? 'application/x-www-form-urlencoded' : $type;
	...
	function _httprequest($url, $fp, $URI, $http_method, $content_type = "", $body = "") {
		...
		if (! empty($this->host) && ! isset($this->rawheaders['Host'])) {
			...
			if (! empty($this->port) && $this->port != '80') $headers .= ":" . $this->port;
			$headers .= "\r\n";
		}
		...
		if (! empty($content_type)) {
			...
			if ($content_type == "multipart/form-data") $headers .= "; boundary=" . $this->_mime_boundary;
			$headers .= "\r\n";
		}
		...
		} elseif (is_array($this->results)) $this->results[] = $results;  // have we already fetched framed content?
		// no framed content
	...
	function setcookies() {
		...
		for($x = 0; $x < count($this->headers); $x ++) {
			if (preg_match('/^set-cookie:[\s]+([^=]+)=([^;]+)/i', $this->headers[$x], $match)) $this->cookies[$match[1]] = urldecode($match[2]);
		}
	...
	function _connect(&$fp) {
		...
		if ($this->scheme == 'https') {
			...
			if (isset($this->cafile) || isset($this->capath)) {
				...
				if (isset($this->cafile)) $context_opts['ssl']['cafile'] = $this->cafile;
				if (isset($this->capath)) $context_opts['ssl']['capath'] = $this->capath;
			}
		...
		if (version_compare(PHP_VERSION, '5.0.0', '>')) {
			if ($this->scheme == 'http') $host = "tcp://" . $host;
	...
	function _prepare_post_body($formvars, $formfiles) {
		...
		if (empty($formvars) && count($formfiles) == 0) return '{}';
		// end.
		switch ($this->_submit_type) {
			case 'application/json':
				if (is_array($formvars)) {
					$postdata = rjson_encode($formvars);
				} else {
					$postdata = $formvars;
				}
			break;
			case "application/x-www-form-urlencoded" :
				...
				while (list($key, $val) = each($formvars)) {
					...
					} else $postdata .= urlencode($key) . "=" . urlencode($val) . "&";
				}
			break;
				...
				while (list($field_name, $file_names) = each($formfiles)) {
					...
					while (list(, $file_name) = each($file_names)) {
						...
						if (is_array($file_name)) {
							...
							} elseif (isset($file_name['tmp_name'])) {
								$base_name = $file_name['name'];
