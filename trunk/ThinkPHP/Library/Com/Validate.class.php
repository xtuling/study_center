...
/**
 * thinkphp5 验证器
 * 官方文档: http://www.kancloud.cn/manual/thinkphp5/129352
 *
 * 引入者: zhoutao
 * 按照功能有些thinkphp5的方法调用改为了现有框架的方法, 其他没有改变
 *
 * Date: 2016/12/21
 * Time: 上午11:02
 */
...
class Validate
{
     ...
     * @return bool
     */
    protected function requireWithNone($value, $rule, $data)
    {
        $val = $this->getDataValue($data, $rule);
        if (empty($val)) {
            return !empty($value);
    ...
    protected function token($value, $rule, $data)
    {
        ...
        if (!isset($data[$rule]) || is_null(session($rule))) {
            // 令牌数据无效
            return false;
        }

        // 令牌验证
        if (isset($data[$rule]) && session($rule) === $data[$rule]) {
            // 防止重复提交
            session($rule, null); // 验证完成销毁session
            return true;
        }
        // 开启TOKEN重置
        session($rule, null);
