...
class PHPRPC_Server {
	...
	function initKey($key = '') {
		...
		if (isset($_SESSION[$this->cid])) {
			...
			$this->key = md5(empty($key) ? cfg('PHPRPC_SECRET') : $key).md5(substr($this->cid, 7));
			$this->encrypt = true;
			require_once ('xxtea.php');
			return;
		}
	...
	function callFunction() {
		...
		if (array_key_exists($function, $this->functions)) {
			// by zhuxun, 先调用 before_action 方法;
			$func_name = $function;
			if (array_key_exists('before_action', $this->functions)) {
				$params_ba = array($func_name);
				$this->call($this->functions['before_action'], $params_ba);
			}
			// end.
			...
			// by zhuxun, 程序执行完, 调用 after_action 方法;
			if (array_key_exists('after_action', $this->functions)) {
				$param_aa = array($func_name);
				$this->call($this->functions['after_action'], $param_aa);
			}
			// end.
