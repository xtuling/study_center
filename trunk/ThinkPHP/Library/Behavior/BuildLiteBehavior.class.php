...
class BuildLiteBehavior
{
    ...
    public function run(&$params)
    {
        ...
        // 读取编译列表文件
        $filelist = is_file(CONF_PATH . 'lite.php') ? include CONF_PATH . 'lite.php' : array(
        ...
        $content .= "\n";
        $content .= 'namespace { ';
        $content .= 'Think\Think::addMap(' . var_export(\Think\Think::getMap(), true) . ');';
        $content .= "\n";
        $content .= 'L(' . var_export(L(), true) . ');';
        $content .= "\n";
        $content .= 'C(' . var_export(C(), true) . ');';
        $content .= 'Think\Hook::import(' . var_export(\Think\Hook::get(), true) . ');';
        $content .= 'Think\Think::start();';
        $content .= '}';
