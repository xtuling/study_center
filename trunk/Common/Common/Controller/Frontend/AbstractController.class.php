...
abstract class AbstractController extends Controller
{
    ...
    protected function _userLogin()
    {
        ...
        if (empty($this->_login->user) && $this->_require_login) {
            // 未从 Cookie 中获取到用户信息，自动登陆
            $login_success = $this->_login->autoLogin();
    ...
    protected function _output($tpl)
    {
        ...
        $this->view->assign('title', 'loading ...');
        // 域名信息
        $this->view->assign('domain', isset($this->_setting['domain']) ? $this->_setting['domain'] : QY_DOMAIN);
        // 输出当前用户信息
